-- FlowFit Phase 2: reconciliação aditiva do contrato de treinos e convites.
-- Esta migration preserva sessões legadas (set_number NULL) e não remove dados.
...
update public.workout_set_logs
   set workout_exercise_id = exercise_id
...
begin
  ...
  if jsonb_array_length(p_exercises) > 50 then
    raise exception 'workout_too_many_exercises';
  end if;
  ...
  loop
    ...
    v_media_type := lower(trim(coalesce(
      v_exercise ->> 'media_type',
      case when v_media_url = '' then 'none' else 'external' end
    )));
...
create or replace function public.validate_student_invite(
  p_token text,
...
revoke all on function public.validate_student_invite(text, text) from public, authenticated;
...
revoke all on function public.claim_student_access(text) from public, anon;
